...
DEFUSE_apply = function(Combined_X, Combined_Y, source_index, miss_source, imput_method, seednum) {
  ...
  for (ff in 1:3) {
    ...
    weight_lc_1 = density_ratio_xgboost_linear(
      Combined_X[source_index[[3]],][, covariate_z],                          # UC X
      Combined_X[source_index[[1]],][fold_id == density_id, covariate_z],     # LC train (density_id)
      Combined_X[source_index[[1]],][fold_id == score_id,    covariate_z]     
    )
    weight_lc[fold_id == score_id] = weight_lc_1
    mod_1 <- suppressWarnings(
      glm(
    ...
    for (col in 1:ncol(X_complete)) {
      ...
      f1_DR_check_cond_UC[, col] <- f1_DR_check_cond_UC[, col] + predict(fit_col, newdata = Combined_X[source_index[[3]],][, covariate_z, drop = FALSE])
    }
    ...
    for (col in 1:ncol(X_complete)) {
      for (row in col:ncol(X_complete)) {
        ...
        pred_uc <- predict(fit_rc, newdata = Combined_X[source_index[[3]],][, covariate_z, drop = FALSE])

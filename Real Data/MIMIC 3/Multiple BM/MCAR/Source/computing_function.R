...
### MCAR calibration
allocation = function(method, phi_1_C, phi_1_IC, S, miss, X_C, X_IC, Y_C, Y_IC) {
  n_C = nrow(phi_1_C)
  n_IC = nrow(phi_1_IC)
  phi_1 = rbind(phi_1_C, phi_1_IC)
  S_cali = S - phi_1[1:n_C, ]
  
  if (method == "intercept") {
    XY_C =matrix(1, nrow = n_C, ncol = 1)
    XY_IC = matrix(1, nrow = n_IC, ncol = 1)
  } else if (method == "linear") {
    XY_C = as.matrix(cbind(X_C[, -miss], Y_C, 1))
    XY_IC = as.matrix(cbind(X_IC[, -miss], Y_IC, 1))
    colnames(XY_C) = c()
    colnames(XY_IC) = c()
  } else if (method == "interaction") {
    XY_C =  as.matrix(cbind(X_C[, -miss], Y_C, 1, X_C[, -miss] * Y_C))
    XY_IC =  as.matrix(cbind(X_IC[, -miss], Y_IC, 1, X_IC[, -miss] * Y_IC))
    colnames(XY_C) = c()
    colnames(XY_IC) = c()
  }
  
  delta = matrix(0, nrow = ncol(XY_C), ncol = ncol(S))
  
  for (k in 1:ncol(S)) {
    tem_X = as.matrix(phi_1[1:n_C, k] * XY_C, ncol = ncol(XY_C))
    numerator = cov(tem_X, S_cali[, k])
    denominator = cov(tem_X)
    delta[, k] = solve(denominator) %*% as.matrix(numerator, ncol = 1)
  }
  weight_CIC = as.matrix(rbind(XY_C, XY_IC), ncol = ncol(XY_C)) %*% delta + 1
  adjusted_phi_1 = as.matrix(phi_1, ncol = ncol(phi_1)) * weight_CIC
  return(list(adjusted_phi_1 = adjusted_phi_1, weight = weight_CIC, delta = delta))
}

block_weight = function(phi_C_list, phi_IC_list, S) {
  list_length = length(phi_C_list)
  alpha_1 <- matrix(0, ncol = ncol(S), nrow = list_length)
  for (j in 1:ncol(S)) {
    denominator = matrix(0, list_length, list_length)
    numerator = matrix(0, nrow = list_length, ncol = 1)
    
    for (i in 1:list_length) {
      phi_C1 = phi_C_list[[i]][, j]
      phi_IC1 = phi_IC_list[[i]][, j]
      for (k in 1:list_length) {
        phi_C2 = phi_C_list[[k]][, j]
        phi_IC2 = phi_IC_list[[k]][, j]
        if (i == k) {
          rho = length(phi_C1) / length(phi_IC1)
          denominator[i, k] = cov(phi_C1, phi_C2) + rho * cov(phi_IC1, phi_IC2)
        } else {
          denominator[i, k] = cov(phi_C1, phi_C2)
        }
      }
      numerator[i] = cov(phi_C1, S[, j])
    }
    alpha_1[, j] = solve(denominator) %*% numerator
  }
  return(list(alpha_1 = alpha_1))
}
### 
allocation2 = function(method, phi_2_C, phi_2_UC, S1, X_C, X_UC) {
  n_C = nrow(phi_2_C)
  n_UC = nrow(phi_2_UC)
  phi_2 = rbind(phi_2_C, phi_2_UC)
  S_cali = S1 - phi_2[1:n_C, ]
  
  if (method == "intercept") {
    X_C = matrix(1, nrow = n_C, ncol = 1)
    X_UC = matrix(1, nrow = n_UC, ncol = 1)
  } else if (method == "linear") {
    X_C = as.matrix(cbind(X_C, 1))
    X_UC = as.matrix(cbind(X_UC, 1))
    colnames(X_C) = c()
    colnames(X_UC) = c()
  } 
  delta = matrix(0, nrow = ncol(X_C), ncol = ncol(S1))
  for (k in 1:ncol(S)) {
    tem_X = as.matrix(phi_2[1:n_C, k] * X_C, ncol = ncol(X_C))
    numerator = cov(tem_X, S_cali[, k])
    denominator = cov(tem_X)
    delta[, k] = solve(denominator) %*% as.matrix(numerator, ncol = 1)
  }
  weight_CUC = as.matrix(rbind(X_C, X_UC), ncol = ncol(X_C)) %*% delta + 1
  adjusted_phi_2 = as.matrix(phi_2, ncol = ncol(phi_2)) * weight_CUC
  
  return(list(adjusted_phi_2 = adjusted_phi_2, weight = weight_CUC, delta = delta))
}

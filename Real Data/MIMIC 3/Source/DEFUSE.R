DEFUSE_apply = function(Combined_X, Combined_Y, source_index, miss_source, imput_method, seednum, A) {
  ...
  f1_DR_check_cond_UC = matrix(0, nrow = n[5], ncol = length(A))
  j1_DR_check_cond_LC = array(0, dim = c(n[1], length(A), length(A)))
  j1_DR_check_cond_UC = array(0, dim = c(n[5], length(A), length(A)))
  ...
  for (ff in 1:3) {
    ...
    score_id = (ff + 1) %% 3
    hessian_id = (ff + 2) %% 3
    weight_lc_1 = density_ratio_xgboost_linear(Combined_X[source_index[[5]], ][, covariate_z], Combined_X[source_index[[1]], ][fold_id == density_id, covariate_z], Combined_X[source_index[[1]], ][fold_id == score_id, covariate_z])
    weight_lc[fold_id == score_id] = weight_lc_1
    mod_1 <- suppressWarnings(glm(Y_complete[fold_id == score_id] ~ X_complete[fold_id == score_id, A] - 1, weights = weight_lc_1, family = binomial()))
    gamma_check = gamma_check + mod_1$coefficients
    f1_DR_check_1 = X_complete[, A] * as.numeric(Y_complete - g_x(X_complete, mod_1$coefficients))
    j1_DR_check_1 = H_array(X_complete[, A], mod_1$coefficients)
    X_tr_score <- X_complete[fold_id == score_id, covariate_z, drop = FALSE]
    for (col in 1:length(A)) {
      ...
      invisible(capture.output({
        fit_col <- gausspr(x = X_tr_score, y = Y_tr_score, kernel = "rbfdot", kpar = list(sigma = best_score_hp$sigma), var = best_score_hp$var)
      }))
      f1_DR_check_cond_LC[fold_id == hessian_id, col] <- predict(fit_col, newdata = X_complete[fold_id == hessian_id, covariate_z, drop = FALSE])
      f1_DR_check_cond_UC[, col] <- f1_DR_check_cond_UC[, col] + predict(fit_col, newdata = Combined_X[source_index[[5]], ][, covariate_z, drop = FALSE])
    }
    X_tr_hess <- X_complete[fold_id == hessian_id, covariate_z, drop = FALSE]
    for (col in 1:length(A)) {
      for (row in col:length(A)) {
        ...
        invisible(capture.output({
          fit_rc <- gausspr(x = X_tr_hess, y = Y_tr_hess, kernel = "rbfdot", kpar = list(sigma = best_hess_hp$sigma), var = best_hess_hp$var)
        }))
        pred_test <- predict(fit_rc, newdata = X_complete[fold_id == density_id, covariate_z, drop = FALSE])
        j1_DR_check_cond_LC[fold_id == density_id, row, col] <- pred_test
        j1_DR_check_cond_LC[fold_id == density_id, col, row] <- pred_test
        pred_uc <- predict(fit_rc, newdata = Combined_X[source_index[[5]], ][, covariate_z, drop = FALSE])
  ...
  J_tilt_inv_lc <- solve(H(Combined_X[source_index[[5]], A], gamma_tilt, rep(1, n[5])))

...
discrepancy = function(Y_LC, X_LC, X_LM, Y_LM, X_UC, seednum) {
  ...
  for (iLC in 1:n_fold) {
    ...
    best_hp_dml_LC <- tune_gausspr_cv_auto(X_LC[fold_id_LC == dml_id, covariate_z],
    ...
    best_hp_dml_LM <- tune_gausspr_cv_auto(X_LM[fold_id_LM == dml_id, covariate_z], 
  ...
  res1 = selection_combination(as.matrix(dml_m_LM_UC[train_id_UC == 0, ]), as.matrix(dml_m_LC_UC[train_id_UC == 0, ]),
                               weight_LM[train_id_LM == 0], Y_LM[train_id_LM == 0], as.matrix(dml_m_LM_LM[train_id_LM == 0, ]),
                               weight_LC[train_id_LC == 0], Y_LC[train_id_LC == 0], as.matrix(dml_m_LC_LC[train_id_LC == 0, ]),
                               V_UC[train_id_UC == 0, ], V_LM[train_id_LM == 0, ], V_LC[train_id_LC == 0, ])
  res2 = selection_combination(as.matrix(dml_m_LM_UC), as.matrix(dml_m_LC_UC),
                               weight_LM, Y_LM, as.matrix(dml_m_LM_LM),
                               weight_LC, Y_LC, as.matrix(dml_m_LC_LC),
                               V_UC, V_LM, V_LC)
  res3 = selection_eval(as.matrix(dml_m_LM_UC[train_id_UC == 1, ]), as.matrix(dml_m_LC_UC[train_id_UC == 1, ]),
                               weight_LM[train_id_LM == 1], Y_LM[train_id_LM == 1], as.matrix(dml_m_LM_LM[train_id_LM == 1, ]),
                               weight_LC[train_id_LC == 1], Y_LC[train_id_LC == 1], as.matrix(dml_m_LC_LC[train_id_LC == 1, ]),
                               V_UC[train_id_UC == 1, ], V_LM[train_id_LM == 1, ], V_LC[train_id_LC == 1, ], res1$alpha)

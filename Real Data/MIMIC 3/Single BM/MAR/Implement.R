...
sum_dat = rbind(psychiatric_labeled, psychiatric_unlabeled) %>%
  ...
  mutate(l50993 = as.numeric(scale(l50993)), l51233 = as.numeric(scale(l51233)), recent_age = as.numeric(scale(recent_age)))
...
## data modification
X_lic1 <- psychiatric_labeled %>% dplyr::select(recent_age, HDL, diabetes, l50993, l51233, 9:ncol(psychiatric_labeled)) %>% filter(!is.na(l50993), is.na(l51233))
X_lic2 <- psychiatric_labeled %>% dplyr::select(recent_age, HDL, diabetes, l50993, l51233, 9:ncol(psychiatric_labeled)) %>% filter(is.na(l50993), !is.na(l51233))
X_lic3 <- psychiatric_labeled %>% dplyr::select(recent_age, HDL, diabetes, l50993, l51233, 9:ncol(psychiatric_labeled)) %>% filter(is.na(l50993), is.na(l51233))
X_lic = rbind(X_lic1, X_lic2, X_lic3)
X_lic$HDL <- ifelse(X_lic$HDL > 0, 1, 0)
X_lic$diabetes <- ifelse(X_lic$diabetes > 0, 1, 0)
X_lc <- psychiatric_labeled %>% dplyr::select(recent_age, HDL, diabetes, l50993, l51233, 9:ncol(psychiatric_labeled)) %>% filter(!is.na(l50993) & !is.na(l51233))
X_lc$HDL <- ifelse(X_lc$HDL > 0, 1, 0)
X_lc$diabetes <- ifelse(X_lc$diabetes > 0, 1, 0)
X_uc <- psychiatric_unlabeled %>% dplyr::select(recent_age, HDL, diabetes, l50993, l51233, 9:ncol(psychiatric_unlabeled)) %>% filter(!is.na(l50993), !is.na(l51233))
...
X_lc = X_lc %>% select(select_covariates)
X_lic1 = X_lic1 %>% select(select_covariates)
X_lic2 = X_lic2 %>% select(select_covariates)
X_lic3 = X_lic3 %>% select(select_covariates)
X_uc = X_uc %>% select(select_covariates)
for (taskid in 1:1) {
  n_lc = nrow(X_lc)
  n_lic1 = nrow(X_lic1)
  n_lic2 = nrow(X_lic2)
  n_lic3 = nrow(X_lic3)
  n_uc = nrow(X_uc)
  source_index = list(c(1:n_lc), c((n_lc + 1):(n_lc + n_lic1)), c((n_lc + n_lic1 + 1):(n_lc + n_lic2 + n_lic2)), c((n_lc + n_lic1 + n_lic2 + 1):(n_lc + n_lic1 + n_lic2 + n_lic3)), c((n_lc + n_lic1 + n_lic2 + n_lic3 + 1):(n_lc + n_lic1 + n_lic2 + n_lic3 + n_uc)))
  miss_source = list(NULL, 5, 4, c(4, 5), NULL)
  nrep = 1
  ...
  reduces_res_table3 = matrix(0, nrow = nrep, ncol = 5)
  res_check = matrix(0, nrow = nrep, ncol = 5)
  ssl_table = matrix(0, nrow = nrep, ncol = 5)
  survey_table = matrix(0, nrow = nrep, ncol = 5)
  ...
  reduced_var_table_uc1 = matrix(0, nrow = nrep, ncol = 5)
  reduced_var_table_uc2 = matrix(0, nrow = nrep, ncol = 5)
  A = 1:5
  for (i in 1:nrep) {
    tryCatch({

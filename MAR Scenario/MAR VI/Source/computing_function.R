...
### Density ration using lasso selection
density_ratio_lasso = function(target, source, evaluate, A) {
  combined_Y = c(rep(0, nrow(source)), rep(1, nrow(target)))
  combined_X = rbind(source, target)
  interaction = NULL
  for (i in 1:length(A)) {
    interaction = cbind(interaction, combined_X[, A[i]] * combined_X[, -A])
  }
  combined_X = cbind(combined_X, combined_X^2, interaction)
  ...
  for (i in 1:length(A)) {
    interaction_test = cbind(interaction_test, evaluate[, A[i]] * evaluate[, -A])
  }
  X_test = cbind(1, evaluate, evaluate^2, interaction_test)
  pred_prob <- 1 / (1 + exp(- X_test %*% coef(cvfit, s = "lambda.min")))
  odds = pred_prob  / (1 - pred_prob)
  w = as.numeric(odds * nrow(evaluate) / nrow(target))
  weight = w / mean(w)
  return(weight)

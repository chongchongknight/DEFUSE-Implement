...
main_func <- function(Combined_X, Combined_Y, n, S_DR_tilt, gamma_tilt, 
  ...
  K1_cond_Z_LM = matrix(0, nrow = nrow(X_LM), ncol = length(A))
  K1_cond_Z_UC = matrix(0, nrow = nrow(X_UC), ncol = length(A))
  K1_cond_XgammaY_LC = matrix(0, nrow = nrow(X_LC), ncol = length(A))
  K1_cond_XgammaY_LM = matrix(0, nrow = nrow(X_LM), ncol = length(A))
  K1_cond_XgammaY_UC = matrix(0, nrow = nrow(X_UC), ncol = length(A))
  K1_cond_XgammaY_Z_LC = matrix(0, nrow = nrow(X_LC), ncol = length(A))
  K1_cond_XgammaY_Z_LM = matrix(0, nrow = nrow(X_LM), ncol = length(A))
  K1_cond_XgammaY_Z_LC_UC = matrix(0, nrow = nrow(X_UC), ncol = length(A))
  K1_cond_XgammaY_Z_LM_UC = matrix(0, nrow = nrow(X_UC), ncol = length(A))
  weight_LC = rep(0, n[1])
  weight_LM = rep(0, n[2])
  ...
  for (col in 1:length(A)) {
    ...
    for (ff in 1:4) {
      ...
      weight_lc = density_ratio_lasso(X_UC[ ,XOmega], 
                                      X_LC[fold_id_LC == density_id, XOmega], 
                                      X_LC[fold_id_LC == score_xsuby_id, XOmega],
                                      A)
      weight_LC[fold_id_LC == score_xsuby_id] = weight_lc
      X_LC_tr_score_z <- X_LC[fold_id_LC == score_z_id, XOmega]
      K1_tr_score_z <- K1[fold_id_LC == score_z_id]
      XY_LC_tr_score_xsuby <- cbind(X_LC[fold_id_LC == score_xsuby_id, XGamma], Y_LC[fold_id_LC == score_xsuby_id])
      ...
      best_hess_hp_score_xsuby <- tune_gausspr_cv(XY_LC_tr_score_xsuby, K1_tr_score_score_xsuby) #tuning parameter selection
      ...
      K1_cond_XgammaY_LC[fold_id_LC == score_xsuby_z_id, col] = predict(mod_K1_XgammaY_LC, newdata = cbind(X_LC[fold_id_LC == score_xsuby_z_id, XGamma], Y_LC[fold_id_LC == score_xsuby_z_id]))
      X_LC_tr_score_xsuby_z <- X_LC[fold_id_LC == score_xsuby_z_id, XOmega]
      ...
      K1_cond_XgammaY_Z_LC[fold_id_LC == score_z_id, col] = predict(mod_K1_XgammaY_Z_LC, newdata = X_LC[fold_id_LC == score_z_id, XOmega])
      K1_cond_Z_LC[fold_id_LC == density_id, col] = predict(mod_K1_Z_LC, newdata = X_LC[fold_id_LC == density_id, XOmega])
      K1_cond_Z_UC[, col] = predict(mod_K1_Z_LC, newdata = X_UC[,XOmega]) + K1_cond_Z_UC[, col]
      K1_cond_XgammaY_LM[, col] = predict(mod_K1_XgammaY_LC, newdata = cbind(X_LM[,XGamma], Y_LM)) + K1_cond_XgammaY_LM[, col]
      K1_cond_XgammaY_Z_LC_UC[, col] = predict(mod_K1_XgammaY_Z_LC, newdata = X_UC[,XOmega]) + K1_cond_XgammaY_Z_LC_UC[, col]
    }
    ...
    for (ff in 1:2) {
      ...
      weight_lm = density_ratio_lasso(X_UC[ ,XOmega], 
                                      X_LM[fold_id_LM == density_id, XOmega], 
                                      X_LM[fold_id_LM == score_xsuby_z_id, XOmega],
                                      A)
      weight_LM[fold_id_LM == score_xsuby_z_id] = weight_lm
      X_LM_tr_score_xsuby_z <- X_LM[fold_id_LM == score_xsuby_z_id, XOmega]
      ...
      K1_cond_XgammaY_Z_LM[fold_id_LM == density_id, col] = predict(mod_K1_XgammaY_Z_LM, newdata = X_LM[fold_id_LM == density_id, XOmega])
      K1_cond_XgammaY_Z_LM_UC[, col] = predict(mod_K1_XgammaY_Z_LM, newdata = X_UC[,XOmega]) + K1_cond_XgammaY_Z_LM_UC[, col]
    }
    K1_cond_XgammaY_Z_LM_UC[, col] = K1_cond_XgammaY_Z_LM_UC[, col] / 2
  }
  rho1 = n[2] / n[1]
  ## calibration weight alpha
  alpha = matrix(0, nrow = nK2, ncol = length(A))
  for (col in 1:length(A)) {
    var1 = cov(weight_LC * (K1_cond_XgammaY_LC[, col] * cbind(1, X_LC[, A]) - K1_cond_XgammaY_Z_LC[, col] * cbind(1, X_LC[, A])))
    var2 = cov(weight_LM * (K1_cond_XgammaY_LM[, col] * cbind(1, X_LM[, A]) - K1_cond_XgammaY_Z_LM[, col] * cbind(1, X_LM[, A])))
    cov1 = cov(weight_LC * (S_DR_tilt[, col] - K1_cond_Z_LC[, col]), 
               weight_LC * (K1_cond_XgammaY_LC[, col] * cbind(1, X_LC[, A]) - K1_cond_XgammaY_Z_LC[, col] * cbind(1, X_LC[, A])))
    alpha[, col] = solve(var1 + 1 / rho1 * var2) %*% as.vector(cov1)
  }
  M_phi1_LC = weight_LC * (sapply(seq_len(length(A)), function(k) K1_cond_XgammaY_LC[, k] * cbind(1, X_LC[, A]) %*% alpha[ , k]) - 
                             sapply(seq_len(length(A)), function(k) K1_cond_XgammaY_Z_LC[, k] * cbind(1, X_LC[, A]) %*% alpha[ , k])) + 
    t(matrix(rep(colMeans(sapply(seq_len(length(A)), function(k) K1_cond_XgammaY_Z_LC_UC[, k] * cbind(1, X_UC[, A]) %*% alpha[ , k])), n[1]), nrow = length(A)))
  M_phi1_LM = weight_LM * (sapply(seq_len(length(A)), function(k) K1_cond_XgammaY_LM[, k] * cbind(1, X_LM[, A]) %*% alpha[ , k]) - 
                             sapply(seq_len(length(A)), function(k) K1_cond_XgammaY_Z_LM[, k] * cbind(1, X_LM[, A]) %*% alpha[ , k])) + 
    t(matrix(rep(colMeans(sapply(seq_len(length(A)), function(k) K1_cond_XgammaY_Z_LM_UC[, k] * cbind(1, X_UC[, A]) %*% alpha[ , k])), n[2]), nrow = length(A)))
  ## gamma1
  gamma_1_ad = gamma_tilt - colMeans(M_phi1_LC) + colMeans(M_phi1_LM)
  ## incorporate unlabel data
  ## initial setup
  S1 = S_DR_tilt - sapply(seq_len(length(A)), function(k) K1_cond_XgammaY_LC[, k] * cbind(1, X_LC[, A]) %*% alpha[ , k])
  nT2 = 1 + ncol(X_LC[, A])
  ...
  for (col in 1:ncol(S1)) {
    ...
    for (ff in 1:4) {
      ...
      X_LC_tr_score_x_z <- X_LC[fold_id_LC == score_x_z_id, XOmega]
      ...
      T1_cond_X_Z_LC[fold_id_LC == score_z_id, col] = predict(mod_T1_X_Z_LC, newdata = X_LC[fold_id_LC == score_z_id, XOmega])
      T1_cond_Z_LC[fold_id_LC == density_id, col] = predict(mod_T1_Z_LC, newdata = X_LC[fold_id_LC == density_id, XOmega])
      T1_cond_X_UC[, col] = predict(mod_T1_X_LC, newdata = X_UC[, ]) + T1_cond_X_UC[, col]
      T1_cond_X_Z_LC_UC[, col] = predict(mod_T1_X_Z_LC, newdata = X_UC[, XOmega]) + T1_cond_X_Z_LC_UC[, col]
    }
  ...
  zeta = matrix(0, nrow = nT2, ncol = length(A))
  for (col in 1:length(A)) {
    var1 = cov(weight_LC * (T1_cond_X_LC[, col] * cbind(1, X_LC[, A]) - T1_cond_X_Z_LC[, col] * cbind(1, X_LC[, A])))
    var2 = cov(T1_cond_X_UC[, col] * cbind(1, X_UC[, A]))
    cov1 = cov(weight_LC * (S1[, col] - T1_cond_Z_LC[, col]),
               weight_LC * (T1_cond_X_LC[, col] * cbind(1, X_LC[, A]) - T1_cond_X_Z_LC[, col] * cbind(1, X_LC[, A])))
    zeta[, col] = solve(var1 + 1 / rho2 * var2) %*% as.vector(cov1)
  }
  M_phi2_LC = weight_LC * (sapply(seq_len(length(A)), function(k) T1_cond_X_LC[, k] * cbind(1, X_LC[, A]) %*% zeta[ , k]) - 
                             sapply(seq_len(length(A)), function(k) T1_cond_X_Z_LC[, k] * cbind(1, X_LC[, A]) %*% zeta[ , k])) + 
    t(matrix(rep(colMeans(sapply(seq_len(length(A)), function(k) T1_cond_X_Z_LC_UC[, k] * cbind(1, X_UC[, A]) %*% zeta[ , k])), n[1]), nrow = length(A)))
  phi2_UC = sapply(seq_len(length(A)), function(k) T1_cond_X_UC[, k] * cbind(1, X_UC[, A]) %*% zeta[ , k]) 
  gamma_2_ad = gamma_1_ad - colMeans(M_phi2_LC) + colMeans(phi2_UC)
  ## without calibration or zero weight
  alpha0 = matrix(0, nrow = nK2, ncol = length(A))
  zeta0 = matrix(0, nrow = nT2, ncol = length(A))

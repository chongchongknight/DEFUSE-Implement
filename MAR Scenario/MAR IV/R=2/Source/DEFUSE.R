...
DEFUSE_apply = function(Combined_X, Combined_Y, source_index, miss_source, imput_method, seednum) {
  ...
  f1_DR_check_cond_UC = matrix(0, nrow = n[4], ncol = dim)
  j1_DR_check_cond_LC = array(0, dim = c(n[1], dim, dim))
  j1_DR_check_cond_UC = array(0, dim = c(n[4], dim, dim))
  ...
  for (ff in 1:3) {
    ...
    weight_lc_1 = density_ratio_xgboost_linear(
      Combined_X[source_index[[4]],][, 1:3],                          # UC X
    ...
    for (col in 1:ncol(X_complete)) {
      ...
      f1_DR_check_cond_UC[, col] <- f1_DR_check_cond_UC[, col] + predict(fit_col, newdata = Combined_X[source_index[[4]],][, 1:3, drop = FALSE])
    }
    ...
    for (col in 1:ncol(X_complete)) {
      for (row in col:ncol(X_complete)) {
        ...
        pred_uc <- predict(fit_rc, newdata = Combined_X[source_index[[4]],][, 1:3, drop = FALSE])
  ...
  J_tilt_inv_lc <- solve(H(Combined_X[source_index[[4]],], gamma_tilt, rep(1, n[4])))

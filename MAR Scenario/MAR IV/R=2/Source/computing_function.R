...
var_cal_2 = function(S_DR_tilt, K1_cond_Z_LC, 
  ...
  if (nrow(zeta) > 1) {
    res = colVars(weight_LC * (S1 - T1_cond_Z_LC) -
      ...
      1 / rho3 * colVars(sapply(seq_len(ncol(S_DR_tilt)), function(k) T1_cond_X_UC[, k] * cbind(1, X_UC[, covariate_z]) %*% zeta[ , k]))
  } else {
    res = colVars(as.numeric(weight_LC) * (S1 - T1_cond_Z_LC) -
      ...
      1 / rho3 * colVars(T1_cond_X_UC %*% diag(as.numeric(zeta)))
  }
  return(res)
}

...
mix_gaussian <- function(n_site, n_total, n_label, mu, sigma, imput_linear, y_correct, M, gamma.bar, seednum,
  ...
  set.seed(seednum)
  error_table = seq(-mean_shift, mean_shift, length.out = n_site)
  ...
  for (i in 1:n_site) {
    ...
    if (imput_linear == 1) {
      x[[i]][,M[1]] = x[[i]][,-M] %*% c(0.5, 0.5, 0.5) + rnorm(n_total[i],sd = 1)
      x[[i]][,M[2]] = x[[i]][,-M] %*% c(0.5, 0.5, 0.5) + rnorm(n_total[i],sd = 1)
    } else {
      x[[i]][,M[1]] = rnorm(n_total[i]) + sin(x[[i]][,-M] %*% c(0.5, 0.5, 0.5))
      x[[i]][,M[2]] = rnorm(n_total[i]) + sin(x[[i]][,-M] %*% c(0.5, 0.5, 0.5))
    }
    if (y_correct == 1) {

### calculation of gamma1 and gamma2 for different calibration and conditional models CM
main_func <- function(Combined_X, Combined_Y, n, S_DR_tilt, gamma_tilt, source_index, seednum){
  ...
  K1_cond_Z_LM2 = matrix(0, nrow = nrow(X_LM2), ncol = ncol(X_LM2))
  K1_cond_Z_UC = matrix(0, nrow = nrow(X_UC), ncol = ncol(X_UC))
  K1_cond_XgammaY1_LC = matrix(0, nrow = nrow(X_LC), ncol = ncol(X_LC))
  K1_cond_XgammaY2_LC = matrix(0, nrow = nrow(X_LC), ncol = ncol(X_LC))
  K1_cond_XgammaY1_LM1 = matrix(0, nrow = nrow(X_LM1), ncol = ncol(X_LM1))
  K1_cond_XgammaY2_LM2 = matrix(0, nrow = nrow(X_LM2), ncol = ncol(X_LM2))
  K1_cond_XgammaY1_UC = matrix(0, nrow = nrow(X_UC), ncol = ncol(X_UC))
  K1_cond_XgammaY2_UC = matrix(0, nrow = nrow(X_UC), ncol = ncol(X_UC))
  K1_cond_XgammaY1_Z_LC = matrix(0, nrow = nrow(X_LC), ncol = ncol(X_LC))
  K1_cond_XgammaY2_Z_LC = matrix(0, nrow = nrow(X_LC), ncol = ncol(X_LC))
  K1_cond_XgammaY1_Z_LM1 = matrix(0, nrow = nrow(X_LM1), ncol = ncol(X_LM1))
  K1_cond_XgammaY2_Z_LM2 = matrix(0, nrow = nrow(X_LM2), ncol = ncol(X_LM2))
  K1_cond_XgammaY1_Z_LC_UC = matrix(0, nrow = nrow(X_UC), ncol = ncol(X_UC))
  K1_cond_XgammaY2_Z_LC_UC = matrix(0, nrow = nrow(X_UC), ncol = ncol(X_UC))
  K1_cond_XgammaY1_Z_LM1_UC = matrix(0, nrow = nrow(X_UC), ncol = ncol(X_UC))
  K1_cond_XgammaY2_Z_LM2_UC = matrix(0, nrow = nrow(X_UC), ncol = ncol(X_UC))
  weight_LC = rep(0, n[1])
  ...
  alpha = matrix(0, nrow = nK2 * 2, ncol = ncol(S_DR_tilt))
  for (col in 1:ncol(S_DR_tilt)) {
  ...
  ## gamma1
  gamma_1_ad = gamma_tilt - colMeans(M_phi1_LC1)  - colMeans(M_phi1_LC2) + colMeans(M_phi1_LM1) + colMeans(M_phi1_LM2)
  ...
  S1 = S_DR_tilt - sapply(seq_len(ncol(S_DR_tilt)), function(k) cbind(K1_cond_XgammaY1_LC[, k] * cbind(1, X_LC[, covariate_z]),
                                                                      K1_cond_XgammaY2_LC[, k] * cbind(1, X_LC[, covariate_z])) %*% alpha[ , k])
  ...
  M_phi2_LC = weight_LC * (sapply(seq_len(ncol(S_DR_tilt)), function(k) T1_cond_X_LC[, k] * cbind(1, X_LC[, covariate_z]) %*% zeta[ , k]) - 
    ...
    t(matrix(rep(colMeans(sapply(seq_len(ncol(S_DR_tilt)), function(k) T1_cond_X_Z_LC_UC[, k] * cbind(1, X_UC[, covariate_z]) %*% zeta[ , k])), n[1]), nrow = 5))
  ...
  alpha0 = matrix(0, nrow = 2 * nK2, ncol = ncol(S_DR_tilt))
  zeta0 = matrix(0, nrow = nT2, ncol = ncol(S_DR_tilt))
  var_tilt = var_cal(S_DR_tilt, K1_cond_Z_LC,
                     weight_LC, weight_LM1, weight_LM2, alpha0, rho1, rho2, 
                     K1_cond_XgammaY1_LC, K1_cond_XgammaY2_LC,
                     K1_cond_XgammaY1_Z_LC, K1_cond_XgammaY2_Z_LC,
                     K1_cond_XgammaY1_LM1, K1_cond_XgammaY1_Z_LM1, 
                     K1_cond_XgammaY2_LM2, K1_cond_XgammaY2_Z_LM2,
                     X_LC, X_LM1, X_LM2, covariate_z, nK2)
  var_ad1 = var_cal(S_DR_tilt, K1_cond_Z_LC,
                    weight_LC, weight_LM1, weight_LM2, alpha, rho1, rho2, 
                    K1_cond_XgammaY1_LC, K1_cond_XgammaY2_LC,
                    K1_cond_XgammaY1_Z_LC, K1_cond_XgammaY2_Z_LC,
                    K1_cond_XgammaY1_LM1, K1_cond_XgammaY1_Z_LM1, 
                    K1_cond_XgammaY2_LM2, K1_cond_XgammaY2_Z_LM2,
  ...
  T_part_var0 = var_cal_2(S_DR_tilt, K1_cond_Z_LC, 
                          S1, T1_cond_Z_LC,
                          weight_LC, zeta0, rho3,
                          T1_cond_X_LC, T1_cond_X_Z_LC, 
                          T1_cond_X_UC,
                          X_LC, X_UC, covariate_z)
...
### calculation of gamma1 and gamma2 for different calibration and reduced models RM
main_func2 <- function(Combined_X, Combined_Y, n, S_DR_tilt, gamma_tilt, source_index, seednum){
  ...
  for (col in 1:ncol(S_DR_tilt)) {
    ...
    for (ff in 1:3) {
      ...
      mod_K1_XgammaY1_LC = lm(Y_LC ~ X_LC[, covariate_lic1] - 1)
      K1_cond_XgammaY1_LC = t(t(X_LC) %*% X_LC[, covariate_lic1] %*% solve(t(X_LC[, covariate_lic1]) %*% X_LC[, covariate_lic1]) %*% t(X_LC[, covariate_lic1] * as.numeric(mod_K1_XgammaY1_LC$residuals)))
      K1_cond_XgammaY1_LM1 = t(t(X_LC) %*% X_LC[, covariate_lic1] %*% solve(t(X_LC[, covariate_lic1]) %*% X_LC[, covariate_lic1]) %*% t(X_LM1[, covariate_lic1] * as.numeric(Y_LM1 - X_LM1[, covariate_lic1] %*% mod_K1_XgammaY1_LC$coefficients)))
      ## best linear projection for the reduced model, simple parametric without cross-fitting
      mod_K1_XgammaY2_LC = lm(Y_LC ~ X_LC[, covariate_lic2] - 1)
      K1_cond_XgammaY2_LC = t(t(X_LC) %*% X_LC[, covariate_lic2] %*% solve(t(X_LC[, covariate_lic2]) %*% X_LC[, covariate_lic2]) %*% t(X_LC[, covariate_lic2] * as.numeric(mod_K1_XgammaY2_LC$residuals)))
      K1_cond_XgammaY2_LM2 = t(t(X_LC) %*% X_LC[, covariate_lic2] %*% solve(t(X_LC[, covariate_lic2]) %*% X_LC[, covariate_lic2]) %*% t(X_LM2[, covariate_lic2] * as.numeric(Y_LM2 - X_LM2[, covariate_lic2] %*% mod_K1_XgammaY2_LC$coefficients)))
      invisible(capture.output({
        mod_K1_Z_LC = gausspr(X_LC_tr_score_z, 
                              K1_tr_score_z, 
                              kernel = "rbfdot", 
      ...
      X_LC_tr_score_xsuby_z <- X_LC[fold_id_LC == score_xsuby1_z_id, covariate_z]
      K1_tr_score_xsuby1_z <- K1_cond_XgammaY1_LC[fold_id_LC == score_xsuby1_z_id, col]
      K1_tr_score_xsuby2_z <- K1_cond_XgammaY2_LC[fold_id_LC == score_xsuby2_z_id, col]
      best_hess_hp_score_xsuby1_z <- tune_gausspr_cv(X_LC_tr_score_xsuby_z, K1_tr_score_xsuby1_z)
      best_hess_hp_score_xsuby2_z <- tune_gausspr_cv(X_LC_tr_score_xsuby_z, K1_tr_score_xsuby2_z)
      invisible(capture.output({
        mod_K1_XgammaY1_Z_LC = gausspr(X_LC_tr_score_xsuby_z, 

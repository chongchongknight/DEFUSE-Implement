...
## sample size, number of misaligned source
parameter_list = list(
  c(1000, 1),
  c(1000, 2),
  c(1000, 3),
...
## 1 for misaligned, 0 for aligned
label = t(matrix(c(0, 0, 0, 1,
                   0, 1, 0, 1,
                   0, 1, 1, 1), nrow = 4))
...
for (cc in 1:n_cc) {
  ...
  for (i in 1:nrep) {
    ...
    dat = mix_gaussian(n_site = n_site, n_total = n_total, dim = dim, Miss = Miss, 
                       gamma.bar = gamma.bar, seednum = seednum, size = para1, nmissing = para2)
    ...
    suppressMessages({
      AUC_F1_subtable[i, 1] = auc(roc(label[para2, ], as.numeric(res$keep_id1), levels = c(0, 1), direction = "<"))
      AUC_F1_subtable[i, 2] = auc(roc(label[para2, ], as.numeric(res$keep_id2), levels = c(0, 1), direction = "<"))
      AUC_F1_subtable[i, 3] = auc(roc(label[para2, ], as.numeric(res$keep_id3), levels = c(0, 1), direction = "<"))
      AUC_F1_subtable[i, 4] = auc(roc(label[para2, ], as.numeric(res$keep_id4), levels = c(0, 1), direction = "<"))
    })
    AUC_F1_subtable[i, 5] = f1_score(label[para2, ], as.numeric(res$keep_id1))
    AUC_F1_subtable[i, 6] = f1_score(label[para2, ], as.numeric(res$keep_id2))
    AUC_F1_subtable[i, 7] = f1_score(label[para2, ], as.numeric(res$keep_id3))
    AUC_F1_subtable[i, 8] = f1_score(label[para2, ], as.numeric(res$keep_id4))
  }
}
